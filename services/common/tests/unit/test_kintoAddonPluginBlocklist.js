...
function* clear_state() {
  ...
  Services.prefs.clearUserPref(PREF_KINTO_GFX_CHECKED_SECONDS);
...
add_task(function* test_sends_reload_message_when_addons_has_changes(){
  ...
  let waitForMessage = new Promise((resolve, reject) => {
    ...
    AddonClient.maybeSync(2000, Date.now() - 1000);
  });
...
add_task(function* test_sends_reload_message_when_plugins_has_changes(){
  ...
  let waitForMessage = new Promise((resolve, reject) => {
    ...
    PluginClient.maybeSync(2000, Date.now() - 1000);
  });
  yield waitForMessage;
...
add_task(function* test_sends_reload_message_when_gfx_has_changes(){
  ...
  let waitForMessage = new Promise((resolve, reject) => {
    ...
    GfxClient.maybeSync(2000, Date.now() - 1000);
  });
  yield waitForMessage;
